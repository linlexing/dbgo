...
app.controller('mainCtrl', ['$translate', '$scope','$alert','$http','$timeout',function ($translate, $scope,$alert,$http,$timeout) {
	...
	$scope.fetchData=function(){
		if($scope.data.data.length>0){
			fetchOption.lastkey = _.pick(_.last($scope.data.data),$scope.sort.column,primaryKeys);
		}
		$scope.fetchInfo = {
			startTime:new Date(),
		}
		$scope.pending ++;
		websocket.send({event:"rv_fetchData",data:fetchOption}) ;
		/*$http.post(dataAction,fetchOption)
